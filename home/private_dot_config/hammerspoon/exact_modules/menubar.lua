...
function drawInfo:refreshBattInfo()
	...
	self.capacity = batt_info.capacity

	if precent == self.percentage and isAC == self.isAC and isCharging == self.isCharging then
		self.refreshed = false
	else
		print(string.format("Battery: (%d) (%d/%s)", batt_info.percentage, batt_info.capacity, batt_info.maxCapacity))
		self.refreshed = true
...
function drawInfo:computeIconParams()
	...
	if self.isAC then
		frameHeight = self.isCharging and 24 or 21
	end
...
CW = hcw.new(function(event_type)
	...
	if event_type == hcw.screensDidLock then
		hs.execute("/opt/homebrew/bin/nowplaying-cli pause")
		BW:stop()
		LST:stop()

...
mvn() {
  ...
  if curl -s --connect-timeout 1 http://127.0.0.1:8081 >/dev/null; then
    echo "Use local nexus" >&2
    extra_args+=("-Pnexus-mine")
  fi

  $mvnw_path \
    -Dmaven.repo.local=$XDG_CACHE_HOME/maven \
    -s $MAVEN_USER_HOME/settings.xml \
    "${extra_args[@]}" \
    "$@"
}
alias mvnw=mvn
...
upload() {
  curl -u inx:wudile -T "$1" https://webdav.x
}

function rssh() {
  local remote_port=${1:-55555}
  local local_port=${2:-8082}
  local host=${2:-kuailai}
...
de() {
  ...
  docker-compose -f "${XDG_CONFIG_HOME}/compose/develop-compose.yml" "$@"
}
